...
export class UserVehiclesComponent implements OnChanges, OnInit {
  ...
  updateVehicles(){
    this.vehicleService.getAllAvailableVehicles().then(result => this.vehiclesInDropDown = this.mapToAvailableVehicles(result));
  }
  ...
  private mapToAvailableVehicles(result: VehiclesDTO) {
    return result.vehicles.map(r => new AvailableVehicle(r.vehicleId,r.make,r.model,r.productionYear))
  }
