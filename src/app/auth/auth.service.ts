...
export class AuthService {
  ...
  private sendCredentials(
    ...
    return this.http
      .post<AuthResultDTO>(environment.apiHost + "login", {
  ...
  private persist(loginResult: AuthResultDTO) {
    this.authUserSessionStorageService.persist(
      new AuthUserSessionRecord(loginResult.token, loginResult.expiresIn)
    );
  }
